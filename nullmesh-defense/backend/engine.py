...
class NullMeshEngine:
    ...
    def snapshot(self, t: int) -> TickReport:
        ...
        for h in self.current_histories:
            for w in W:
                new_h.add(h + (w,))
                if len(new_h) > 5000:
                    raise MemoryError("State space explosion")
    ...
    def run(self, t0: int, t_end: int, break_early: bool = True) -> CDTResult:
        reports: list[TickReport] = []
        t_star = None
        first_violated: tuple[InvariantId, ...] = ()
        cause = None
...
def doctrine_must_complete(action_A: Action, action_B: Action, deadline_idx: int) -> Invariant:
    # If A happens, B must happen by the deadline_idx (which is len(h) - 1)
    def holds(h: History) -> bool:
        idx = len(h) - 1
        has_A = any(act == action_A for w in h for ag, act in w.assignment)
        if not has_A:
            return True
            
        if idx < deadline_idx:
            # Deadline hasn't arrived, still valid
            return True
            
        # Deadline has passed or arrived. B must have happened.
        has_B = any(act == action_B for w in h for ag, act in w.assignment)
        return has_B
        
    return Invariant("INV-011", f"doctrine: if {action_A}, must {action_B}", holds)


# v2 Advanced Warfare Invariants

def exactly_one_leader(agents: list[AgentId], leadership_action: str) -> Invariant:
    def holds(h: History) -> bool:
        w = h[-1]
        leader_count = 0
        for ag, act in w.assignment:
            if ag in agents and act == leadership_action:
                leader_count += 1
        return leader_count == 1
    return Invariant("INV-012", f"swarm_topology: exactly 1 leader performing '{leadership_action}'", holds)


def emission_safebound(real_asset: AgentId, real_hide_action: str, decoy_asset: AgentId, decoy_emit_action: str) -> Invariant:
    def holds(h: History) -> bool:
        w = h[-1]
        real_is_hiding = False
        decoy_is_emitting = False
        
        for ag, act in w.assignment:
            if ag == real_asset and act == real_hide_action:
                real_is_hiding = True
            if ag == decoy_asset and act == decoy_emit_action:
                decoy_is_emitting = True
                
        # If the real asset hides, the decoy MUST emit to maintain the signature.
        if real_is_hiding and not decoy_is_emitting:
            return False
        return True
    return Invariant("INV-013", f"ew_safebound: if {real_asset} hides, {decoy_asset} must emit", holds)


def supply_chain_integrity(dependent_action: Action, provider_action: Action) -> Invariant:
    def holds(h: History) -> bool:
        w = h[-1]
        needs_supply = False
        has_supply = False
        ...
        for ag, act in w.assignment:
            if act == dependent_action:
                needs_supply = True
            if act == provider_action:
                has_supply = True
                
        if needs_supply and not has_supply:
            return False
        return True
    return Invariant("INV-014", f"logistics: {dependent_action} requires {provider_action}", holds)


def quantum_entanglement_integrity(emitter_action: Action, receiver_action: Action, intercept_action: Action) -> Invariant:
    def holds(h: History) -> bool:
        w = h[-1]
        is_emitting = False
        is_receiving = False
        ...
        for ag, act in w.assignment:
            if act == emitter_action: is_emitting = True
            if act == receiver_action: is_receiving = True
            if act == intercept_action: is_intercepting = True
        if is_emitting and is_receiving and is_intercepting:
            return False
        return True
    return Invariant("INV-015", f"quantum_entanglement: intercepted {emitter_action} by {intercept_action}", holds)


def functional_redundancy_integrity(required_paths: int, link_actions: list[Action]) -> Invariant:
    def holds(h: History) -> bool:
        w = h[-1]
        active_paths = 0
        for ag, act in w.assignment:
            if act in link_actions:
                active_paths += 1
        return active_paths >= required_paths
    return Invariant("INV-016", f"redundancy: requires {required_paths} of {link_actions}", holds)


def zero_trust_isolation(cloud_process_action: Action, sever_action: Action) -> Invariant:
    def holds(h: History) -> bool:
        w = h[-1]
        cloud_reliant = False
        is_severed = False
        for ag, act in w.assignment:
            if act == cloud_process_action: cloud_reliant = True
            if act == sever_action: is_severed = True
        if cloud_reliant and is_severed:
            return False
        return True
    return Invariant("INV-017", f"zero_trust: {cloud_process_action} fails if {sever_action}", holds)


def continuous_authentication(auth_action: Action, protected_action: Action) -> Invariant:
    def holds(h: History) -> bool:
        w = h[-1]
        is_authenticated = False
        is_accessing = False
        for ag, act in w.assignment:
            if act == auth_action: is_authenticated = True
            if act == protected_action: is_accessing = True
        if is_accessing and not is_authenticated:
            return False
        return True
    return Invariant("INV-018", f"zero_trust_auth: {protected_action} requires {auth_action}", holds)


def partial_mesh_resilience(min_active_nodes: int, node_agents: list[AgentId], routing_action: Action) -> Invariant:
    def holds(h: History) -> bool:
        w = h[-1]
        active_nodes = 0
        for ag, act in w.assignment:
            if ag in node_agents and act == routing_action:
                active_nodes += 1
        return active_nodes >= min_active_nodes
    return Invariant("INV-019", f"partial_mesh: at least {min_active_nodes} nodes must perform {routing_action}", holds)


def sdn_rerouting_integrity(controller_action: Action, disrupt_action: Action) -> Invariant:
    def holds(h: History) -> bool:
        w = h[-1]
        is_disrupted = False
        is_rerouting = False
        for ag, act in w.assignment:
            if act == disrupt_action: is_disrupted = True
            if act == controller_action: is_rerouting = True
            
        if is_disrupted and not is_rerouting:
            return False
        return True
    return Invariant("INV-020", f"sdn: {controller_action} required if {disrupt_action}", holds)


def killer_demo_contracts() -> list[IntentContract]:
    return [
        IntentContract("A", {0: frozenset({"claim:alpha"}), 12: frozenset({"claim:alpha"})}),
        IntentContract("B", {0: frozenset({"claim:bravo"}), 12: frozenset({"claim:bravo"})}),
