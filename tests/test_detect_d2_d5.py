...
def test_fixture_expected(session, name, detector):
    ...
    assert Counter((r.detector_id, r.title) for r in results if r.severity == "FAIL") == Counter((r["detector_id"], r["title"]) for r in expected["fail"])
...
def test_d5_creation_still_checked_without_modification(session, tmp_path):
    ...
    assert Counter(r.severity for r in results) == {"UNKNOWN": 1, "FAIL": 3}
...
def test_d2_strict_date_rules(session, created, modified, expected):
    ...
    entry.is_void = True  # voiding does not erase mutation evidence
    session.flush()
...
def test_d2_clear_date_without_flag_and_payload(session):
    ...
    assert result.payload == dict(cleared_date="2025-03-31", modified_at="2025-06-02T00:00:00",
                                 created_at="2025-01-15T00:00:00", debit=50000, credit=0, account_id="1000")
...
def test_ingest_failure_removes_metadata(session, tmp_path, monkeypatch):
    ...
    assert list((tmp_path / "data").rglob("ingest_metadata.json")) == []
