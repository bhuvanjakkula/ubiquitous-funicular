"""Replay, run D1/D2/D4/D5, and commit their proposals and findings."""
...
def run_d1_d2_d4_d5(session, job_id) -> dict:
    # Replay commits its own checkpoint. Detection is a separate transaction.
    replay_job(session, job_id)
    try:
        findings = run_d1(session, job_id) + run_d2(session, job_id) + run_d4(session, job_id) + run_d5(session, job_id)
